...
SaveData_Path = strcat(folder_ProcessedData,'FluxStressCalcs');
...
for i = 1:2
    ...
    ylabel('u_{*,thr} (m/s)');
    set(gca,'FontSize',16);
    h_legend = legend({'data','original fit','Jasper fit','original sqrt','Jasper sqrt'},'Location','NorthWest');
    set(h_legend,'FontSize',16);
    xlim([0.3 0.55]);
    ylim([0.24 0.36]);
end
...
for i = 1:N_Sites;
    ...
    errorbar(ust_ex_bins_mid,zbar_ex_bins_mean,zbar_ex_bins_SE,Markers{i});
end

%add Greeley and Namikas data
ust_ex_Greeley96 = ust_mpers_Greeley96-polyval(Pfit_ustThr_d50_sqrt,sqrt(d50_mm_Greeley96));
zbar_nondim_Greeley96 = 1e3*zbar_m_Greeley96/d50_mm_Greeley96;
plot(ust_ex_Greeley96,zbar_nondim_Greeley96,'^k');
ust_ex_Namikas03 = ust_mpers_Namikas03-polyval(Pfit_ustThr_d50_sqrt,sqrt(d50_mm_Namikas03));
zbar_nondim_Namikas03 = 1e3*zbar_m_Namikas03/d50_mm_Namikas03;
plot(ust_ex_Namikas03,zbar_nondim_Namikas03,'dk');
